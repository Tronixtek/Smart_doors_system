...
public class TtlockModule extends ReactContextBaseJavaModule {
    private static final String DEBUG_TAG = "TtlockModule";
...
//        initSDK();
    ...
    private void prepareBtServices(Activity activity) {
        if (activity == null) {
            Log.w(DEBUG_TAG, "prepareBtServices skipped because current activity is null");
            return;
        }

        Log.d(DEBUG_TAG, "Preparing TTLock Bluetooth services");
                    ...
                    for (int i=0;i<permissions.length;i++) {
                        if (grantResults[i] == PackageManager.PERMISSION_GRANTED) {
                            if (Manifest.permission.ACCESS_FINE_LOCATION.equals(permissions[i])
                                    || Manifest.permission.BLUETOOTH_SCAN.equals(permissions[i])
...
//        scanGateway = true;
      ...
      PermissionUtils.doWithScanPermission(getCurrentActivity(), success -> {
        if (success) {
          prepareBtServices(getCurrentActivity());
...
//        ExtendedBluetoothDevice device = mCachedDevice.get(mac);
    ...
    public void startScan() {
        ...
        Log.d(DEBUG_TAG, "startScan() called");
//        scanGateway = false;
        scanType = TYPE_LOCK;
        PermissionUtils.doWithScanPermission(getCurrentActivity(), success -> {
          Log.d(DEBUG_TAG, "startScan permission result: " + success);
          if (success) {
            Activity activity = getCurrentActivity();
            Log.d(DEBUG_TAG, "startScan current activity: " + activity);
            prepareBtServices(activity);
            TTLockClient.getDefault().stopScanLock();
            Log.d(DEBUG_TAG, "Invoking TTLockClient.startScanLock()");
...
//                LogUtil.d("extendedBluetoothDevice:" + extendedBluetoothDevice);
                if(extendedBluetoothDevice != null){
                  Log.d(
                    DEBUG_TAG,
                    "onScanLockSuccess mac=" + extendedBluetoothDevice.getAddress()
                      + " name=" + extendedBluetoothDevice.getName()
                      + " rssi=" + extendedBluetoothDevice.getRssi()
                      + " isSettingMode=" + extendedBluetoothDevice.isSettingMode()
                      + " isTouch=" + extendedBluetoothDevice.isTouch()
                  );
              ...
              public void onFail(LockError error) {
                ...
                Log.e(DEBUG_TAG, "startScanLock onFail: " + error);
              }
            });
          } else {
            Log.w(DEBUG_TAG, "startScan() aborted because scan permission callback returned false");
          }
        });
    }
